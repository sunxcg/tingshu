...
//        }
    ...
    override fun getCategoryList(url: String): Category {
        val doc = Jsoup.connect(url).config().get()
        val nextUrl = doc.select(".getMore > a").firstOrNull { it.text().contains("下页") }?.attr("abs:href") ?: ""
        val pages = doc.selectFirst(".getMore").ownText().let { text ->
    ...
    override fun headers(audioUrl: String): Map<String, String> {
        ...
        if (audioUrl.contains("ysxs8.com")) {
