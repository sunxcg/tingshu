...
object M56TingShu : TingShu() {
    override fun getSourceId(): String {
        return "0efa840c460f43a1a7b41d54a7f084c4"
    }
    ...
    override fun getCategoryMenus(): List<CategoryMenu> {
        ...
        val doc = Jsoup.connect("http://m.ting56.com/mulu.html").config(false).get()
        doc.select(".chan_box").forEach { box ->
            val title = box.selectFirst("h2").text()
            val subMenu = box.select("ul > li > a").map { category ->
                CategoryTab(category.text(), category.absUrl("href"))
            }
